...
public class StringTool {
		...
		if (strValue != null && !strValue.equals("")) {
			...
			for (int nloop = 0; nloop < strValue.length(); nloop++) {
				if (strValue.charAt(nloop) == ' ') {
					...
					if (!Pattern.compile("^(([A-F])*([a-f])*(\\d)*)$").matcher(strValue.substring(nloop, nloop + 1))
							.matches()) {
						return strAryResult;
					}
					...
					if ((nTemp == nLen)
							|| (nloop == strValue.length() - 1 && (strTemp != null && !strTemp.equals("")))) {
	 ...
	 * 
	 * @param c
	 *            the needed converter character.
